...
!!! note "PowerShell security"
    If you get a security error, you may need to run this command in PowerShell first:
    ```powershell
...
1. **Your domain slug** — a short, lowercase, hyphenated identifier for your primary knowledge area (e.g. `my-domain`, `support-kb`, `platform`). This becomes the default domain in your first articles.
...
Just open the `my-kb` folder in VS Code, Vim, or whatever you use. Everything is plain Markdown.

No plugins are required. You can use any AI CLI tool that can read files and run shell commands in the KB directory.
...
- **Keep articles fresh** → articles with a past `review_due:` date will be flagged automatically
...
**`build_catalog.py` exits with "No articles found"**
:   You haven't ingested any sources yet. Run `ingest-source` on a file in `00 Raw Sources/` first.

**Article frontmatter validation errors from `verify_catalog.py`**
